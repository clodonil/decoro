...
def index():
    ...
    session['tela'] = "cliente"
    return render_template('cliente/index.html',title='Lista de clientes',todos=todos)


@cliente.route('/new', methods=['GET','POST'])
@login_required
def new():
    form = EmpForm()
...
@cliente.route("/edit/<int:emp_id>", methods = ["GET","POST"])
@login_required
def edit(emp_id):
